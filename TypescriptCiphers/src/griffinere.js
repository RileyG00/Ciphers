...
export class Griffinere {
    ...
    static getDefaultAlphabet() {
        return 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
    }
    static validateAlphabet(alphabet, key) {
    ...
    static createAlphabetPositionMap(alphabet) {
        const map = new Map();
    ...
    static toBase64CharArray(text) {
        if (!text.trim())
            throw new Error('Text cannot be empty');
        const base64 = Buffer.from(text, 'utf8').toString('base64').replace(/=/g, '');
        return base64.split('');
    }
    static fromBase64CharArray(chars) {
        if (chars.length === 0)
    ...
    encryptStringWithMinimumLength(plainText, minimumLength) {
        if (!plainText.trim())
            return '';
        if (minimumLength < 1)
            throw new Error('Minimum response length must be greater than zero.');
        let front = '';
        let back = '';
